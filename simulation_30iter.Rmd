...
sims <- function(simType=1, n=30){
  ...
  if(simType==1){
    for(i in 1:n){
      sim <- linear_sim(n=250, mod_sizes=c(25, 25, 25, 25),
  ...
  CLVClust = CLVObj$partition4$clusters[2,]
    ...
    # kmeansVarClus
      ...
      for (iter in 1:30) {
        ...
        list_crit = c(list_crit, sum(sc@size*sc@withinss)/sum(sc@size))
        list_part_sc[[iter]] =sc
        
        
      }
      
     sc = list_part_sc[[order(list_crit)[1]]]
     sc=sc@.Data
     ScClus <- sc
...
centres = as.numeric(rbind(c1, c2, c3, c4))

centres = matrix(centres, 4,100)
...
for (iter in 1:30) {
  ...
  sc <- specc(t(X), centers=4)
      sc=sc@.Data
      ScClus <- sc
      ARI[[iter]] = randIndex(table(ScClus, partition_ref), adjust = TRUE)
...
consensus_wnmf <- function(M,nb_cluster,option="M",maxiter=30){
  ...
  #W=Poid_init(all_H)
  W = c(0.11, 0.10, 0.12, 0.12, 0.10, 0.11, 0.11, 0.12, 0.10)
  ...
  while(critere_){
    ...
    if(nb_iter==0){
      ...
      ecart_ <- abs(critere_err_i-critere_err)
...
clusters = clusterings
  # For each simulation, matrix with columns=methods and rows=variables:
    ...
    #Initialization
      ...
      WNMFConsWeight=ResWNMF$W #weigth for wnmf
      ...
      RVWeights <- weights #weigth for RVcons
...
```{r data generation}
lin_dat1 <- linear_sim(n=200, mod_sizes=c(25, 25, 25, 25),
                          beta_pos=c(1:3, 76:78),
...
lin_dat2 <- linear_sim(n=200, mod_sizes=c(25, 25, 25, 25),
                          beta_pos=c(1:3, 76:78),
                          beta_vals=c(5, 5, 2, 5, 5, 2), rho=.8, sd=.05)
X2 <- lin_dat2$X
y2 <- lin_dat2$y
...
nLin_dat3 <- nonlinear_sim(n=200, mod_sizes=c(25,25,25,25), rho=.8)
X3 <- nLin_dat3$X
y3 <- nLin_dat3$y
...
```{r}
corrplot(cor(simulation1$Xs[[2]]),method="color", title = name1)
corrplot(cor(simulation2$Xs[[2]]),method="color", title = name2)
corrplot(cor(simulation3$Xs[[2]]),method="color", title = name3)
```
...
for(simulation in list_datasim){
  ...
  for(ii in 1:length(simulation$Xs)){
    ...
    # RF
    ResRF <- randomForest(X,as.vector(y), data=simulation, ntree=1000, keep.forest=FALSE,importance=TRUE)
    result_rf[[ii]] = ResRF
    ...
    partition_ref = rep(1:4, each=25)
    
    ResTotPart=cbind(EnsCl,ResEnsCl[[1]], partition_ref)
    ResTotWeigth = ResEnsCl[[2]]
    ...
    list_ResTotWeigth[[ii]] = ResTotWeigth
    cat("step...", ii, "Exp", E, sep = " ")
  }
  list_resultg = list(result_rf=result_rf, result_ff=result_ff, list_err=list_err,
                      list_ARI=list_ARI, list_ResTotPart=list_ResTotPart, list_ResTotWeigth = list_ResTotWeigth)
...
```{r}
ARIsim1  = list_result_global[[2]]$list_ARI
for (clv in 1:30) {
  X_tocoplot = ARIsim1[[clv]]
  X_tocoplot
  colnames(X_tocoplot) = colnames(ResTotPart)
  rownames(X_tocoplot) = colnames(ResTotPart)
  corrplot(X_tocoplot, method="color", title = name3)
...
nom_partition = c("NbClustWdSil", "CLVClust" ,  "VarClus" ,"DianaClust" ,   "agnesClus" ,    "pamLabels"  ,   "hclusres"  ,    "kmeansvarClus", "ScClus"   , "majVot"  ,      "cspaCons" , "RVCons"    ,    "NMFCons"  ,     "WNMFCons" , "ref")

for (k in 1:30) {
  

partitions = list_result_global[[1]]$list_ResTotPart[[k]]
partitions= cbind(partitions, partition_ref = rep(1:4, each=25))
ResRandAjusted = matrix(NA,15,15)

for(i in 1:dim(partitions)[2]){
      for( j in i:dim(partitions)[2]){
      ResRandAjusted[i,j]=ARI(partitions[,i],partitions[,j])
      ResRandAjusted[j,i]=ResRandAjusted[i,j]
       }
      }
    round(ResRandAjusted,3)
    list_ARI[[k]] = ResRandAjusted
    colnames(ResRandAjusted) = nom_partition
    rownames(ResRandAjusted) = nom_partition
...
partitions = list_result_global[[2]]$list_ResTotPart[[k]]
partitions= cbind(partitions, partition_ref = rep(1:4, each=25))
ResRandAjusted = matrix(NA,15,15)
...
partitions = list_result_global[[3]]$list_ResTotPart[[k]]
partitions= cbind(partitions, partition_ref = rep(1:4, each=25))
ResRandAjusted = matrix(NA,15,15)
...
```{r}
ARIsim2  = list_result_global[[2]]$list_ARI
for (clv in 1:30) {
  
  X_tocoplot = ARIsim2[[clv]]
  X_tocoplot
  colnames(X_tocoplot) = colnames(ResTotPart)
  rownames(X_tocoplot) = colnames(ResTotPart)
  corrplot(X_tocoplot, method="color", title = name3)
...
ARIsim3  = list_result_global[[3]]$list_ARI
for (clv in 1:30) {
  
  X_tocoplot = ARIsim3[[clv]]
  X_tocoplot
  colnames(X_tocoplot) = colnames(ResTotPart)
  rownames(X_tocoplot) = colnames(ResTotPart)
  corrplot(X_tocoplot, method="color", title = name3)
...
weigthsim1  = list_result_global[[2]]$list_ResTotWeigth

for (clv in 1:30) {
  
  X_tocoplot = weigthsim1[[clv]]
  X_tocoplot
  colnames(X_tocoplot) = colnames(ResTotPart)[1:9]
  corrplot(X_tocoplot, method="number", title = name3)
...
weigthsim2  = list_result_global[[2]]$list_ResTotWeigth

for (clv in 1:30) {
  
  X_tocoplot = weigthsim2[[clv]]
  X_tocoplot
  colnames(X_tocoplot) = colnames(ResTotPart)[1:9]
  corrplot(X_tocoplot, method="number", title = name3)
...
weigthsim3  = list_result_global[[3]]$list_ResTotWeigth

for (clv in 1:30) {
  
  X_tocoplot = weigthsim3[[clv]]
  X_tocoplot
  colnames(X_tocoplot) = colnames(ResTotPart)[1:9]
  corrplot(X_tocoplot, method="number", title = name3)
